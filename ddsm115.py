...
class MotorControl:
    ...
    def set_id(self, _id: int):
        ...
        SET_ID = struct.pack(
            self.str_10bytes, 0xAA, 0x55, 0x53, _id, 0x00, 0x00, 0x00, 0x00, 0x00, 0xDE
        )
        for i in range(5):
    ...
    def set_brake(self, _id: int):
        cmd_bytes = struct.pack(
            self.str_9bytes, _id, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00
        )
        cmd_bytes = self.crc_attach(cmd_bytes)
        self.ser.write(cmd_bytes)
        res = self.ser.read_until(size=10)
    ...
    def set_drive_mode(self, _id: int, _mode: int):
        ...
        cmd_bytes = struct.pack(
            self.str_10bytes, _id, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, _mode
        )
        self.ser.write(cmd_bytes)
    ...
    def get_motor_feedback(self, _id: int):
        fb_req_cmd = struct.pack(
            self.str_9bytes, _id, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        )
        ...
        if error != 0:
            ...
            print_warning(
                f"sens_err: {sensor_error} phase_err: {phase_over_error} stall_err: {stall_error} trbs_err: {troubleshoot_error}"
            )
